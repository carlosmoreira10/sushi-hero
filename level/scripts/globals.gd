...
var reputation: int = 50 # 50

func update_reputation(_value: float = 0, _satisfied: bool = false) -> void: #
	if _satisfied:
		if _is_between(0, 40, _value): # entre o tempo de espera
			reputation += randi_range(1, 3) # valor aleatorio no range
	...
	interface.update_reputation(reputation) #erro
	#return

	...
	match _type:
		"table":
			...
			if _table_index == _tables_list.size() and on_sofa:
				update_state("go_away", Vector3.ZERO, Vector3(0, 0, -52))
		...
		"sofa":
			...
			_sofas_list[_sofa_index].is_available(self)
...
func _on_navigation_agent_velocity_computed(_safe_velocity: Vector3) -> void:
	if on_table or on_sofa or getting_up:
		return
...
func _on_wait_timer_timeout():
	...
	getting_up = true # ele vai priorizar a animação de se levantar
	on_table = false
	_update_food_state({})
